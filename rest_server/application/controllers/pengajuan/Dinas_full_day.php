 ...
 * 
 */
class Dinas_full_day extends REST_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('pengajuan/M_Dinas_full_day');
	}
	...
	public function index_get()
	{
		...
		if ($nik_baru === null and $id === null) {
			$user = $this->M_Dinas_full_day->get_index_dinas_full_day();
		} else {
			$user = $this->M_Dinas_full_day->get_index_dinas_full_day($nik_baru, $id);
		}
	...
	public function index_post()
	{
		...
		if ($this->M_Dinas_full_day->createDinas_Full_day($data) > 0) {
			$this->response([
				'status' => true,
	...
	public function index_put()
	{
		...
		if ($this->M_Dinas_full_day->updateApproval($data, $id) > 0 and $this->M_Dinas_full_day->update_data($data2, $shift_day, $badgenumber, $status) > 0) {
			$this->response([
				'status' => true,
	...
	public function index_atasan_get()
	{
		...
		if ($jabatan === null) {
			$team = $this->M_Dinas_full_day->get_index_dinas_full_day_atasan();
		} else {
			$team = $this->M_Dinas_full_day->get_index_dinas_full_day_atasan($jabatan);
		}
	...
	public function index_atasan_lokasi_get()
	{
		...
		$team = $this->M_Dinas_full_day->get_index_full_day_atasan_lokasi($jabatan, $lokasi);
	...
	public function index_dinas_hariini_get()
	{
		...
		$data = $this->M_Dinas_full_day->get_index_dinas_full_day_hari_ini($nik_baru, $tanggal);
	...
	public function index_get_rekap_dinas_full_day_get()
	{
		$id = $this->get('id_full_day');
		$nik_baru = $this->get('nik_baru');

		if ($nik_baru === null and $id === null) {
			$user = $this->M_Dinas_full_day->get_index_rekap_dinas_full_day();
		} else {
			$user = $this->M_Dinas_full_day->get_index_rekap_dinas_full_day($nik_baru, $id);
		}
	...
	public function index_get_dinas_approval_atasan_get()
	{
		$id_divisi = $this->get('id_divisi');
		$id_bagian = $this->get('id_bagian');
		$jabatan   = $this->get('id_jabatan');

		$get = $this->M_Dinas_full_day->get_index_dinas_atasan_new($id_divisi, $id_bagian, $jabatan);
		$totaldata = count($get);
		if ($get) {
			$this->response([
				'status' => true,
